...
type
  ...
  public
    ...
    procedure ScriptReplace(const aRegExpScript: TStrings);
        // Aplica substituições múltiplas de acordo com um script contendo uma ou mais Expressões Regulares
...
{ Cada expressão regular vem em uma linha, logo, cada uma delas é separada por
  "enters" (#13#10). "ª" e "º" separam o que deve ser encontrado do que deve ser
  substituído. A diferença é que "ª" substitui todas as ocorrências e "º"
  substitui apenas a primeira ocorrência encontrada. "ª" significa ALL (tudo) e
  "º" ONE (um) }
procedure TPerlRegEx.ScriptReplace(const aRegExpScript: TStrings);
var
  i, PosGlobal, PosUnique: Word;
begin
  if aRegExpScript.Count > 0 then
    for i := 0 to Pred(aRegExpScript.Count) do
    begin
      PosGlobal := Pos('ª',aRegExpScript[i]);
      PosUnique := Pos('º',aRegExpScript[i]);

      if PosGlobal > 0 then { Substituição global }
      begin
        RegEx := Copy(aRegExpScript[i],1,Pred(PosGlobal));
        Replacement := Copy(aRegExpScript[i],Succ(PosGlobal),Length(aRegExpScript[i]));
        ReplaceAll;
      end
      else if PosUnique > 0 then { Substituição única }
      begin
        RegEx := Copy(aRegExpScript[i],1,Pred(PosUnique));
        Replacement := Copy(aRegExpScript[i],Succ(PosUnique),Length(aRegExpScript[i]));
        if Match then
          Replace;
      end;
    end;
