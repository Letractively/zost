...
begin
	...
	try
        ...
        TempBusy := Busy;
		try
			if MessageBox(Handle,PChar('A operação que está para ser iniciada DESTRUIRÁ/SUBSTITUIRÁ o banco de dados "' + FFSYGlobals.Configurations.DB_DataBase + '". Esta operação não poderá ser interrompida e nem desfeita. Tem certeza?'),'Tem certeza?',MB_ICONWARNING or MB_YESNO) = idYes then
			begin
				RichEditLog.Clear;
