...
var
    FSCForm_Splash: TFSCForm_Splash;
	HForm1, HForm2: TForm;

implementation

uses
	UFSCForm_Main, UXXXTypesConstantsAndClasses;
...
procedure TFSCForm_Splash.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFSCForm_Splash.FormCreate(Sender: TObject);
begin
    FCanClose := True;
end;
...
procedure TFSCForm_Splash.CloseDelayed(const aSeconds: Byte);
begin
    SetTimer(Handle,IDT_DELAYEDCLOSE,aSeconds * 1000,Classes.MakeObjectInstance(DoCloseDelayed));
end;

procedure TFSCForm_Splash.DoCloseDelayed;
begin
    KillTimer(Handle,TWMTimer(Msg).TimerID);
    Close;
end;

procedure TFSCForm_Splash.CloseHForm(Sender: TObject);
begin
    HForm2.Close;
	HForm1.Close;
end;

procedure TFSCForm_Splash.ShowHForm(Sender: TObject);
const
...
procedure TFSCForm_Splash.LabelVersionDblClick(Sender: TObject);
begin
