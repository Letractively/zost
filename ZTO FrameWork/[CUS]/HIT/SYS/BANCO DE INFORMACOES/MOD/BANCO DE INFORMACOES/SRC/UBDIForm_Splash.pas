...
var
    BDIForm_Splash: TBDIForm_Splash;
	HForm1, HForm2: TForm;
...
procedure TBDIForm_Splash.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TBDIForm_Splash.FormCreate(Sender: TObject);
begin
    FCanClose := True;
end;

procedure TBDIForm_Splash.FormShow(Sender: TObject);
begin
    LabelVersion.Caption := TFileInformation.GetInfo(Application.ExeName,'FULLVERSION').AsString;
end;

procedure TBDIForm_Splash.Image1Click(Sender: TObject);
begin
    if FCanClose then
    	Close;
end;

procedure TBDIForm_Splash.CloseDelayed(const aSeconds: Byte);
begin
    SetTimer(Handle,IDT_DELAYEDCLOSE,aSeconds * 1000,Classes.MakeObjectInstance(DoCloseDelayed));
end;

procedure TBDIForm_Splash.DoCloseDelayed;
begin
    KillTimer(Handle,TWMTimer(Msg).TimerID);
    Close;
end;

procedure TBDIForm_Splash.CloseHForm(Sender: TObject);
begin
    HForm2.Close;
	HForm1.Close;
end;

procedure TBDIForm_Splash.ShowHForm(Sender: TObject);
const
...
procedure TBDIForm_Splash.LabelVersionDblClick(Sender: TObject);
begin
