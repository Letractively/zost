inherited BDOForm_Relatorio_PRO: TBDOForm_Relatorio_PRO
  Caption = 'Gerador de relat'#243'rio de propostas'
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel_Header: TPanel
    inherited Label_DialogDescription: TLabel
      Caption = 
        'Nesta tela '#233' poss'#237'vel gerar o relat'#243'rio de propostas. Configure ' +
        'as op'#231#245'es segundo suas necessidades. Clique o bot'#227'o "Configurar"' +
        ' para definir op'#231#245'es espec'#237'ficas da impress'#227'o, como margens e im' +
        'press'#227'o de cores. Para salvar esta proposta como um conjunto de ' +
        'arquivos, clique o bot'#227'o "Salvar...". Para imprimir imediatament' +
        'e, clique o bot'#227'o "Imprimir agora!" Dentro do relat'#243'rio, links, ' +
        'quando dispon'#237'veis, podem ser clicados para exibir mais informa'#231 +
        #245'es sobre um item.'
    end
  end
  inherited GroupBox_VisualizarImpressao: TGroupBox
    inherited PageControl_OpcoesEFiltragem: TPageControl
      inherited TabSheet_OpcoesDeListagem: TTabSheet
        ExplicitLeft = 4
        ExplicitTop = 24
        ExplicitWidth = 619
        ExplicitHeight = 57
        object Bevel1: TBevel
          Left = 135
          Top = 1
          Width = 2
          Height = 55
          Shape = bsLeftLine
        end
        object RadioButton_Normal: TRadioButton
          Left = 1
          Top = 1
          Width = 113
          Height = 13
          Caption = 'Listagem normal'
          Checked = True
          TabOrder = 0
          TabStop = True
          OnClick = DoRecarregar
        end
        object RadioButton_PorRegiao: TRadioButton
          Left = 1
          Top = 29
          Width = 111
          Height = 13
          Caption = 'Listagem por regi'#227'o'
          TabOrder = 1
          OnClick = DoRecarregar
        end
        object Panel1: TPanel
          Left = 143
          Top = 1
          Width = 120
          Height = 55
          AutoSize = True
          BevelOuter = bvNone
          TabOrder = 2
          object RadioButton_OrdenarPorCodigo: TRadioButton
            Left = 0
            Top = 0
            Width = 113
            Height = 13
            Caption = 'Ordenar por c'#243'digo'
            Checked = True
            TabOrder = 0
            TabStop = True
            OnClick = DoRecarregar
          end
          object RadioButton_OrdenarPorValor: TRadioButton
            Left = 0
            Top = 21
            Width = 113
            Height = 13
            Caption = 'Ordenar por valor'
            TabOrder = 1
            OnClick = DoRecarregar
          end
          object RadioButton_OrdenarPorSituacao: TRadioButton
            Left = 0
            Top = 42
            Width = 120
            Height = 13
            Caption = 'Ordenar por situa'#231#227'o'
            TabOrder = 2
            OnClick = DoRecarregar
          end
        end
        object RadioButton_PorInstalador: TRadioButton
          Left = 1
          Top = 15
          Width = 128
          Height = 13
          Caption = 'Listagem por instalador'
          TabOrder = 3
          OnClick = DoRecarregar
        end
        object RadioButton_PorSituacao: TRadioButton
          Left = 1
          Top = 43
          Width = 121
          Height = 13
          Caption = 'Listagem por situa'#231#227'o'
          TabOrder = 4
          OnClick = DoRecarregar
        end
      end
      inherited TabSheet_OpcoesDeFiltragem: TTabSheet
        ExplicitLeft = 4
        ExplicitTop = 24
        ExplicitWidth = 619
        ExplicitHeight = 57
        object Bevel5: TBevel
          Left = 122
          Top = 1
          Width = 2
          Height = 55
          Shape = bsLeftLine
        end
        object Label_OpcoesDisponiveis: TLabel
          Left = 130
          Top = 1
          Width = 291
          Height = 13
          Cursor = crHandPoint
          Caption = 'Sem op'#231#245'es adicionais de filtragem para a op'#231#227'o selecionada'
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clGreen
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = Label_OpcoesDisponiveisClick
        end
        object Bevel2: TBevel
          Left = 432
          Top = 1
          Width = 2
          Height = 55
          Shape = bsLeftLine
        end
        object Label_SituacoesAExibir: TLabel
          Left = 1
          Top = 1
          Width = 46
          Height = 43
          AutoSize = False
          Caption = 'Situa'#231#245'es a exibir'
          Layout = tlCenter
          Visible = False
          WordWrap = True
        end
        object Shape_SituacoesAExibir: TShape
          Left = 1
          Top = 46
          Width = 255
          Height = 10
          Brush.Color = clActiveCaption
          Brush.Style = bsBDiagonal
          Visible = False
        end
        object CheckBoxContarComGanhas: TCheckBox
          Left = 1
          Top = 1
          Width = 115
          Height = 13
          Caption = 'Com obras ganhas'
          Checked = True
          State = cbChecked
          TabOrder = 0
          OnClick = DoRecarregar
        end
        object CheckBoxContarComPerdidas: TCheckBox
          Left = 1
          Top = 22
          Width = 115
          Height = 13
          Caption = 'Com obras perdidas'
          Checked = True
          State = cbChecked
          TabOrder = 1
          OnClick = DoRecarregar
        end
        object CheckBoxContarComSuspensas: TCheckBox
          Left = 1
          Top = 43
          Width = 115
          Height = 13
          Caption = 'Com obras suspensas'
          Checked = True
          State = cbChecked
          TabOrder = 2
          OnClick = DoRecarregar
        end
        object CheckBox_DA_DATADEENTRADA1: TCheckBox
          Left = 440
          Top = 5
          Width = 87
          Height = 13
          Alignment = taLeftJustify
          Anchors = [akTop, akRight]
          Caption = 'No per'#237'odo de'
          TabOrder = 3
          OnClick = CheckBox_DA_DATADEENTRADA1Click
        end
        object CheckBox_DA_DATADEENTRADA2: TCheckBox
          Left = 491
          Top = 39
          Width = 36
          Height = 13
          Alignment = taLeftJustify
          Anchors = [akTop, akRight]
          Caption = 'at'#233
          TabOrder = 4
          OnClick = CheckBox_DA_DATADEENTRADA1Click
        end
        object DateTimePicker_DA_DATADEENTRADA1: TDateTimePicker
          Left = 533
          Top = 1
          Width = 85
          Height = 21
          Anchors = [akTop, akRight]
          Date = 38260.000000000000000000
          Format = 'dd/MM/yyyy'
          Time = 38260.000000000000000000
          Checked = False
          Enabled = False
          TabOrder = 5
          OnChange = DoRecarregar
        end
        object DateTimePicker_DA_DATADEENTRADA2: TDateTimePicker
          Left = 533
          Top = 35
          Width = 85
          Height = 21
          Anchors = [akTop, akRight]
          BiDiMode = bdLeftToRight
          Date = 38260.000000000000000000
          Format = 'dd/MM/yyyy'
          Time = 38260.000000000000000000
          Checked = False
          Enabled = False
          ParentBiDiMode = False
          TabOrder = 6
          OnChange = DoRecarregar
        end
        object ComboBox_OpcaoDeListagem: TComboBox
          Left = 130
          Top = 15
          Width = 296
          Height = 21
          Style = csDropDownList
          Anchors = [akLeft, akTop, akRight]
          Enabled = False
          ItemHeight = 13
          TabOrder = 7
          OnChange = DoRecarregar
        end
        object CheckListBox_SituacoesAListar: TCheckListBox
          Left = 53
          Top = 1
          Width = 203
          Height = 43
          Hint = 'Clique com o bot'#227'o direito do mouse para ver op'#231#245'es de checagem'
          OnClickCheck = DoRecarregar
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ItemHeight = 13
          Items.Strings = (
            'Item1'
            'Item2'
            'Item3')
          ParentFont = False
          ParentShowHint = False
          PopupMenu = PopupActionBar_OpcoesDeChecagem
          ShowHint = True
          TabOrder = 8
          Visible = False
        end
      end
      inherited TabSheet_OpcoesDeExibicao: TTabSheet
        object Bevel3: TBevel
          Left = 111
          Top = 1
          Width = 2
          Height = 55
          Shape = bsLeftLine
        end
        object Bevel4: TBevel
          Left = 206
          Top = 1
          Width = 2
          Height = 55
          Shape = bsLeftLine
        end
        object Label_PRO_VA_COTACOES: TLabel
          Left = 214
          Top = 1
          Width = 81
          Height = 13
          Anchors = [akTop, akRight]
          Caption = 'Moeda/Cota'#231#245'es'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object CheckBoxExibirPeriodo: TCheckBox
          Left = 1
          Top = 22
          Width = 104
          Height = 13
          Caption = 'Exibir per'#237'odo'
          Checked = True
          State = cbChecked
          TabOrder = 0
          OnClick = DoRecarregar
        end
        object CheckBoxExibirValores: TCheckBox
          Left = 119
          Top = 1
          Width = 81
          Height = 13
          Caption = 'Exibir valores'
          Checked = True
          State = cbChecked
          TabOrder = 1
          OnClick = DoRecarregar
        end
        object CheckBoxExibirTotais: TCheckBox
          Left = 1
          Top = 1
          Width = 104
          Height = 13
          Caption = 'Exibir totais'
          Checked = True
          State = cbChecked
          TabOrder = 2
          OnClick = DoRecarregar
        end
        object CheckBoxExibirInstaladores: TCheckBox
          Left = 1
          Top = 43
          Width = 104
          Height = 13
          Caption = 'Exibir instaladores'
          Checked = True
          State = cbChecked
          TabOrder = 3
          OnClick = DoRecarregar
        end
        object Panel_PRO_TI_MOEDA_VA_COTACOES: TPanel
          Left = 214
          Top = 15
          Width = 81
          Height = 21
          Anchors = [akTop, akRight]
          BevelOuter = bvNone
          TabOrder = 4
          object SpeedButton_PRO_VA_COTACOES: TSpeedButton
            Left = 60
            Top = 0
            Width = 21
            Height = 21
            Cursor = crHandPoint
            Action = Action_DefinirCotacoes
            Flat = True
            Glyph.Data = {
              
              7D00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
            NumGlyphs = 2
          end
          object ComboBox_PRO_TI_MOEDA: TComboBox
            Left = 0
            Top = 0
            Width = 59
            Height = 21
            Style = csDropDownList
            CharCase = ecUpperCase
            ItemHeight = 0
            ItemIndex = 2
            TabOrder = 0
            Text = 'R$'
            OnChange = ComboBox_PRO_TI_MOEDAChange
            Items.Strings = (
              'US$'
              #8364
              'R$'
              #163
              #165)
          end
        end
      end
    end
  end
  inherited ActionList_LocalActions: TActionList
    Images = BDODataModule_Relatorio_PRO.ImageList_Local
    object Action_DefinirCotacoes: TAction
      Hint = 
        'Definir cota'#231#245'es|Clique para definar os valores de cota'#231#227'o para ' +
        'cada moeda'
      ImageIndex = 16
      OnExecute = Action_DefinirCotacoesExecute
    end
    object Action_MarcarTodas: TAction
      Caption = 'Marcar todas'
      Hint = 
        'Marcar todas as situa'#231#245'es|Clique para marcar todas as situa'#231#245'es ' +
        'desta lista'
      OnExecute = Action_MarcarTodasExecute
    end
    object Action_InverterMarcas: TAction
      Caption = 'Inverter marcas'
      Hint = 'Inverter marcas|Clique para inverter as marca'#231#245'es desta lista'
      OnExecute = Action_InverterMarcasExecute
    end
    object Action_SomenteGPS: TAction
      Caption = 'Somente "Ganhas", "Perfidas" e "Suspensas"'
      Hint = 
        'Somente ganhas, perdidas e suspensas|Clique para marcar apenas a' +
        's situa'#231#245'es "Ganha", "Perdida" e "Suspensa"'
      OnExecute = Action_SomenteGPSExecute
    end
    object Action_SomenteG: TAction
      Caption = 'Somente "Ganhas"'
      Hint = 'Somente "Ganhas"|Clique para marcar apenas "Ganhas"'
      OnExecute = Action_SomenteGExecute
    end
    object Action_SomenteP: TAction
      Caption = 'Somente "Perdidas"'
      Hint = 'Somente "Perdidas"|Clique para marcar somente "Perdidas"'
      OnExecute = Action_SomentePExecute
    end
    object Action_SomenteS: TAction
      Caption = 'Somente "Suspensas"'
      Hint = 'Somente "Suspensas"|Clique para marcar somente "Suspensas"'
      OnExecute = Action_SomenteSExecute
    end
  end
  object PopupActionBar_OpcoesDeChecagem: TPopupActionBar
    Left = 30
    Top = 2
    object MenuItem_ChecarTodas: TMenuItem
      Action = Action_MarcarTodas
      RadioItem = True
    end
    object MenuItem_InverterChecagens: TMenuItem
      Action = Action_InverterMarcas
      RadioItem = True
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object MenuItem_SomenteGanhasPerdidasSuspensas: TMenuItem
      Action = Action_SomenteGPS
      RadioItem = True
    end
    object MenuItem_SomenteGanhas: TMenuItem
      Action = Action_SomenteG
      RadioItem = True
    end
    object MenuItem_SomentePerdidas: TMenuItem
      Action = Action_SomenteP
      RadioItem = True
    end
    object MenuItem_SomenteSuspensas: TMenuItem
      Action = Action_SomenteS
      RadioItem = True
    end
  end
end
